...
pixied_generate_find_root() {
    ...
    pixied_die "could not find a Pixi project root from: ${1:-$(pixied_run pwd -P)}"
}
...
pixied_generate_definition() {
    ...
    pixied_die "Pixi project definition is missing from: $root"
}
...
# hook does not start NFS synchronization or a session.
#
...
pixied_generate_direnv_content() {
    ...
    cat <<'ENVRC'
# Generated by pixied generate. Be careful when editing manually.
ENVRC
    printf 'watch_file %s\n' "$definition_name"
    cat <<'ENVRC'
